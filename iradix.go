...
func (t *Txn) initializeWithData(nc *Node, keys [][]byte, searches []int, vals []interface{}) *Node {
	...
	for i := 0; i < len(keys); i++ {
		...
		offset := searches[i]
		if offset == len(key) {
			// Key is fully consumed: attach a leaf here if none is present
			// (or overwrite if you intend to allow duplicates—depends on design).
			if nc.leaf == nil {
				nc.leaf = &leafNode{
					mutateCh: make(chan struct{}),
					key:      key,
		...
		// We still have bytes left in this key; get its current label
		label := key[offset]
		_, child := nc.getEdge(label)

		if child == nil {
			// No existing child for this label => create a brand-new node
			newLeaf := &leafNode{
				mutateCh: make(chan struct{}),
				key:      key,
				val:      vals[i],
			...
			newNode := &Node{
				mutateCh: make(chan struct{}),
				leaf:     newLeaf,
				prefix:   key[offset:], // entire unmatched portion
			}
			// Mark this key as fully inserted
			searches[i] = len(key)
			nc.addEdge(edge{
				label: label,
				node:  newNode,
		...
		// We do have a child => check for common prefix
		childPrefix := child.prefix
		cpLen := longestPrefix(key[offset:], childPrefix)

		if cpLen < len(childPrefix) {
			// We have to split the node
			splitNode := &Node{
				mutateCh: make(chan struct{}),
				prefix:   childPrefix[:cpLen], // the shared portion
			}

			// Replace the old edge with the new splitNode
			nc.replaceEdge(edge{
				label: label,
				node:  splitNode,
			})

			// Restore the existing child on a new edge from splitNode
			// for the "remaining" part of the old child prefix
			modChild := child
			modChild.prefix = childPrefix[cpLen:]
			splitNode.addEdge(edge{
				label: modChild.prefix[0],
				node:  modChild,
			})

			// Now handle the new key
			newLeaf := &leafNode{
				mutateCh: make(chan struct{}),
				key:      key,
				val:      vals[i],
			...
			// Advance the search offset by the common prefix length
			searches[i] = offset + cpLen

			if searches[i] == len(key) {
				// If the key is fully matched by the split prefix
				splitNode.leaf = newLeaf
			} else {
				// Otherwise, create a new edge from the splitNode to a fresh node
				splitNode.addEdge(edge{
					label: key[searches[i]],
					node: &Node{
						mutateCh: make(chan struct{}),
						leaf:     newLeaf,
						prefix:   key[searches[i]:],
					},
				})
			}
			// Mark this key as fully inserted
			...
			// Common prefix covers entire child.prefix or more. No split needed here.
			// We'll handle extension below, in the "grouping" pass, because we need
			// to place the remainder of the key into the subtree of 'child'.
		}
	}

	//-------------------------------------------------------------------
	// The grouping logic: We collect all keys that share the same next
	// label and recurse into the corresponding child node.
	//-------------------------------------------------------------------
	var start int
	for start < len(keys) && searches[start] == len(keys[start]) {
		start++
	}
	if start == len(keys) {
		// All inserted
	...
	startLabel := keys[start][searches[start]]
	subKeys := make([][]byte, 0)
	subVals := make([]interface{}, 0)
	subSearches := make([]int, 0)

	edgeIndex, child := nc.getEdge(startLabel)
	for idx := start; idx < len(keys); {
		// If key is fully consumed, skip
		if searches[idx] == len(keys[idx]) {
			idx++
			continue
		}
		label := keys[idx][searches[idx]]
		if label == startLabel {
			// Accumulate keys that go under the same child
			subKeys = append(subKeys, keys[idx])
			subVals = append(subVals, vals[idx])
			// Advance by however many bytes in the child's prefix
			// so that the child can proceed from the next offset
			subSearches = append(subSearches, searches[idx]+len(child.prefix))
			idx++
		} else {
			// Insert the group that accumulated so far
			newChild := t.initializeWithData(child, subKeys, subSearches, subVals)
			if newChild != nil {
				nc.edges[edgeIndex].node = newChild
			}
			// Reset accumulators
			subKeys = subKeys[:0]
			subVals = subVals[:0]
			subSearches = subSearches[:0]

			// Move 'start' to idx and get the new label
			start = idx
			startLabel = keys[start][searches[start]]
			edgeIndex, child = nc.getEdge(startLabel)
		}
